...
pub const ACCUMULATOR_KEY: [u8; 6] = *b"accuml";
pub const CODE: [u8; 5] = *b":code";
...
impl_runtime_apis! {
	impl crate::HelloWorld<Block> for Runtime {
		fn hello_world() {
			info!(target: "frameless", "🖼️ HellO!");
		}
	}
	...
	impl sp_block_builder::BlockBuilder<Block> for Runtime {
		fn apply_extrinsic(extrinsic: <Block as BlockT>::Extrinsic) -> ApplyExtrinsicResult {
			...
			let acc = sp_io::storage::get(&ACCUMULATOR_KEY);
			let mut accumulator = match acc {
				Some(x) => u8::decode(&mut &*x).expect("We initialized with accumulator, it never got mutated, qed"),
				None => 0u8,
			};
			info!(target: "frameless", "🖼️ Accumulator {:?}",  accumulator);
			...
			info!(target: "frameless", "🖼️ Accumulator after op: {:?}",  accumulator);

			sp_io::storage::set(&ACCUMULATOR_KEY, &accumulator.encode());
		...
		fn inherent_extrinsics(_data: sp_inherents::InherentData) -> Vec<<Block as BlockT>::Extrinsic> {
			...
			let mut v = Vec::new();
			v.push(
				BasicExtrinsic(Calls::Adder(5u8))
			);
			v
		}
...
fn test_apply_extrinsic() {
	let block = BasicExtrinsic::new(Calls::Adder(1u8), None).unwrap();
	info!(target: "frameless", "🖼️ Entering apply_extrinsic: {:?}", Runtime::apply_extrinsic(block));
}
...
// cargo test -p frameless-runtime -- --nocapture // to run without assert false
...
fn test() {
	...
	e.execute_with(|| {
		// let sc = SignedCall { operation: Operation::Add(5), ..SignedCall::default() };
		let e = BasicExtrinsic::new(Calls::Adder(5), None).unwrap();
		println!("{:?}", e.encode());
		Runtime::apply_extrinsic(e);
		println!("{:?}", u32::decode(&mut &*sp_io::storage::get(&ACCUMULATOR_KEY).unwrap()));
		assert_eq!(
			u32::decode(&mut &*sp_io::storage::get(&ACCUMULATOR_KEY).unwrap()).unwrap(),
			5u32
		);

		// let sc = SignedCall { operation: Operation::Add(7), ..SignedCall::default() };
		// let e = BasicExtrinsic::new(Calls::Multiplier(2), None).unwrap();
		// println!("{:?}", e.encode());
		// Runtime::apply_extrinsic(e);
		// assert_eq!(u32::decode(&mut &*sp_io::storage::get(&ACCUMULATOR_KEY).unwrap()).unwrap(), 10);
	});
}
